...
author: 'Peng Liu, Paul Ahlquist, Anthony Gitter, and Irene Ong'
date: '`r Sys.Date()`'
output:
...
```{r, fig.cap='MPAC workflow', out.width='700px', echo=FALSE}
...
- copy-number alteration (CNA): recommend to use data from
    [focal-level](`r focal_url`).
- RNA-seq: both tumor and normal sample data are required. Recommend to use 
...
# Inferred pathway levels (IPLs)

## PARADIGM binary

```{r, echo=FALSE}
prd_paper_url=paste0('https://academic.oup.com/bioinformatics/article/',
    '26/12/i237/282591')
...
MPAC used PARADIGM developed by [Vaske et al.](`r prd_paper_url`) to predict
...
PARADIGM will generate multiple output files. All the file name will start with
...
- Othe auxiliary files, which can be skipped under common usage
...
example, runs on three permutations will generate folders as `p1`, `p2`, and 
`p3`.
...
# Find the largest sub-pathway

MPAC decomposes the original pathway and identify the largest sub-pathway with
all of its entities having filtered IPLs. This sub-pathway allows user to focus
on the most altered pathway network. Note that, because the set of entities 
having filtered IPLs are often different between samples, the sub-pathway will 
largely be different between samples as well and represent sample-specific
features.
...
performs gene set over-represetation analysis on gene with non-zero IPLs in 
the sub-pathway. 
